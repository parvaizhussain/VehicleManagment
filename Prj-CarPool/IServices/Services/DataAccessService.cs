...
namespace Prj_CarPool.IServices.Services
{
	public class DataAccessService : IDataAccessService
	{
		...
		public DataAccessService(ApplicationIdentityDbContext context)
		{
			_context = context;
		}
		...
		public async Task<List<NavigationMenuViewModel>> GetApprovalButtonsAsync(ClaimsPrincipal principal)
		{
			var isAuthenticated = principal.Identity.IsAuthenticated;
			if (!isAuthenticated)
				return new List<NavigationMenuViewModel>();
			...
			var roleIds = await GetUserRoleIds(principal);
			var data = await (from menu in _context.RoleMenuPermission
							  where roleIds.Contains(menu.RoleId) && menu.NavigationMenu.IsButton == true
							  select menu)
							  .Select(m => new NavigationMenuViewModel()
							  {
								  Id = m.NavigationMenu.Id,
								  ...
								  ParentMenuId = m.NavigationMenu.ParentMenuId,
								  Visible = m.NavigationMenu.Visible,
								  StatusCode = m.NavigationMenu.StatusCode,
								  ButtonClass = m.NavigationMenu.ButtonClass
        ...
        public async Task<List<NavigationMenuValueViewModel>> GetPermissionsByRoleValueAsync(string id)
        {
            var items = await (from m in _context.NavigationMenu
                               join rm in _context.RoleMenuPermission
                               on new { X1 = m.Id, X2 = id } equals new { X1 = rm.NavigationMenuId, X2 = rm.RoleId }
                               ...
                               from rm in rmp.DefaultIfEmpty()
                               where rm.RoleId == id
                               select new NavigationMenuValueViewModel()
                               {
                                   value = m.Id,
                                   Name = m.Name,
                                   //Area = m.Area,
