function LoadTable(newdatas) {
    ...
    for (var st = 0; st < newdatas.data.length; st++) {
        ...
        stutbl += '<a href ="javascript:0;" onclick="showdataid(this)" class="text-body" data-bs-toggle="modal" data-bs-target="#modalassigncar" ><i class="ti ti-circle-check ti-sm me-2 approve"></i>Assign</a>';
...
$.ajax({
    type: 'GET',
    async: false,
    url: 'https://localhost:7112/api/VehicleDetails/all',
    success: function (result) {
        VehicleLists += '<option value="0" style="font-weight: bold;background: #d9d5d5;">No Select</option>';
        for (var i = 0; i < result.length; i++) {
            if (result[i].isActive == true && result[i].isDeleted == false) {
                VehicleLists += '<option data-subtext="' + result[i].vehicleNum + '" value="' + result[i].vehicleID + '">' + result[i].vehicleName + '</option>';
               
            }
        }
        $("#VehicleAssign").html(VehicleLists);
...
$.ajax({
    type: 'GET',
    async: false,
    url: 'https://localhost:7112/api/Driver/all',
    success: function (result) {
        DriversLists += '<option value="0" style="font-weight: bold;background: #d9d5d5;">No Select</option>';
        for (var i = 0; i < result.length; i++) {
            if (result[i].isActive == true && result[i].isDeleted == false) {
                DriversLists += '<option data-subtext="' + result[i].driverContact + '" value="' + result[i].driverID + '">' + result[i].driverName + '</option>';

            }
        }
        $("#DriverAssign").html(DriversLists);
...
$.ajax({
    type: 'GET',
    async: false,
    url: 'https://localhost:7112/api/FuelCard/all',
    success: function (result) {
        CardLists += '<option value="0" style="font-weight: bold;background: #d9d5d5;">No Select</option>';
        for (var i = 0; i < result.length; i++) {
            if (result[i].isActive == true && result[i].isDeleted == false) {
                CardLists += '<option data-subtext="' + result[i].cardNum + '" value="' + result[i].cardID + '">' + result[i].cardName + '</option>';

            }
        }
        $("#FuelCardAssign").html(CardLists);
